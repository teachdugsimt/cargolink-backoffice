...
import * as Logo from 'react-icons-kit/fa/';
...
export const Iconsss: React.FC<Omit<IconProps, 'name'> & { name: keyof typeof defaultIcons }> = (props) => {
  return (
    <IconStyle
...
export const ItemIcon: React.FC<ItemIconProps> = ({ icon, className }) => {
  ...
  if (typeof icon === 'string') {
    if (icon in defaultIcons) {
      return <Iconsss name={icon as keyof typeof defaultIcons} className={className} />;
    } else if (evaIcons && icon in evaIcons) {
      return <EvaIcon name={icon as keyof Icons} className={className} />;
    } else {
      return <Icon icon={Logo[icon]} size={18} className={className} />;
    }
