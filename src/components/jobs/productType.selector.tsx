...
const ProductTypeSelector = observer((props: SelectorProps) => {
  ...
  useEffect(() => {
    const listDropdown = JSON.parse(JSON.stringify(versatileStore.list_product_type))
    if(listDropdown) {
      const tmpOptions = [{ label: toFirstLetterUpperCase(placeholder || 'Select'), value: 0 }, ...listDropdown]
      setoptions(tmpOptions)
    }
  }, [JSON.stringify(versatileStore.list_product_type)])
