...
const EditableTextfield = ({ label, value, unit, type, onConfirm }: EditableProps) => (
  <>
    ...
    <RowInline>
      <InlineEdit
        defaultValue={type === 'number' ? value?.replace(',', '') : value}
        editView={({ errorMessage, ...fieldProps }) => (
          // <EditViewContainer>
          <Textfield {...fieldProps} autoFocus type={type} step={type === 'number' ? 0.01 : undefined} />
          // </EditViewContainer>
        )}
        readView={() => (
          <Value>{value}</Value>
...
const TruckDetailWidget = observer((props: TruckWidgetProps) => {
  const {
  ...
  const [tipperOptions, setTipperOptions] = useState<ObjectRoundButton[]>([
    { label: t("DUMP"), value: true, active: tipper ? true : false },
    { label: t("NO_DUMP"), value: false, active: !tipper ? true : false }
  ]);

  useEffect(() => {
    const tipperData = tipperOptions.map((e: ObjectRoundButton) => ({
      ...e,
      active: e.value === tipper
    }))
    console.log('tipperData :>> ', tipperData);
    setTipperOptions(tipperData)
  }, [tipper])
    ...
    <>
      ...
      <Row>
        ...
        <Col flex={1}>
          ...
          <RadioButton list={tipperOptions} onPress={(item: ObjectRoundButton) => onSubmit({ tipper: item.value })} />
        </Col>
      </Row>
